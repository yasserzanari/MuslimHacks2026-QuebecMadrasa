...
type Message = { role: "assistant" | "parent"; title?: string; body: string; sources?: string[]; action?: string; followUps?: { label: string; prompt: string }[]; progressReport?: ProgressReport };
...
export default function ParentAssistantPage() {
  ...
  async function ask(text: string) {
    ...
    const firstDetail = isLesson ? `Je cherche les objectifs disponibles pour ${child.name}.` : isWeek ? `Je lis les créneaux et les activités de ${child.name}.` : `Je vérifie les activités récentes de ${child.name}.`;
    setTrace([
  ...
  async function createLesson() {
    setBusy(true);
    try {
      const childRecordId = childId === "amine" ? "child-adam" : "child-sara";
      const requestId = `assistant-${childRecordId}-${Date.now()}`;
      const response = await fetch("/api/generation-jobs", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ requestId, spec: { childId: childRecordId, locale, subject: "mathematics", level: childId === "amine" ? "primaire-5" : "secondaire-3", format: "practice_set", topic: "Comparer deux fractions", objectives: ["Utiliser un dénominateur commun"], questionTypes: ["short_answer"], questionCount: 5, difficulty: "standard", estimatedMinutes: 25, accommodations: [], context: "", mustInclude: "", mustAvoid: "", includeAnswerKey: true, allowSolutionReveal: false, approvedSourcesOnly: true } }) });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error);
      setJobs((current) => [{ id: data.job?.id ?? requestId, title: data.job?.requestText ?? "Révision de fractions", status: "review_required", credits: data.creditsReserved ?? 1, childName: child.name, createdAt: "à l’instant" }, ...current]);
  ...
  return <main className="parent-assistant-shell parent-ai-shell">
    <ParentSidebar active="assistant" />
    {/* The AI workspace keeps the approved parent assistant layout while sharing the global parent navigation. */}
