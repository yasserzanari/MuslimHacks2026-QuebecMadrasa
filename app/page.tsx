...
export default function HomePage() {
  ...
  return <main className="real-landing">
    ...
    <section className="real-feature-grid" id="fonctionnalites" aria-label={locale === "fr" ? "Fonctionnalités principales" : "Main features"}>{t.features.map(([icon, title, text], index) => <article className="real-feature-card" key={title}><div className="real-feature-icon">{icon}</div><h2>{title}</h2><p>{text}</p><ul>{t.bullets.map((bullet) => <li key={bullet}>{bullet}</li>)}</ul></article>)}</section>
    <section className="real-how section-light" id="fonctionnement"><div><p className="real-eyebrow">{t.weekEyebrow}</p><h2>{t.weekTitle}</h2></div><div className="real-step-list">{t.steps.map(([title, text], index) => <div key={title}><span>0{index + 1}</span><p><strong>{title}</strong><br />{text}</p></div>)}</div></section>
