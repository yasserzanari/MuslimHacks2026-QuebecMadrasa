...
export default function HomePage() {
  ...
  return <main className="real-landing">
    <header className="real-nav-wrap"><nav className="real-nav" aria-label="Main navigation"><Link href="/" className="real-brand"><img className="site-logo-image" src="/ui/logo-madrasa-quebec.png" alt="Madrasa Québec Network" /></Link><div className="real-nav-links">{t.nav.map((label, index) => <a key={label} href={["#familles", "#fonctionnement", "#pods", "#securite", "#tarifs"][index]}>{label}</a>)}</div><div className="real-nav-actions"><Link href="/services/mosquees" className="real-button real-button-outline">Pour les mosquées</Link><Link href="/parent" className="real-button real-button-outline">{t.login}</Link><Link href="/parent" className="real-button real-button-dark">{t.start}</Link></div></nav></header>
    ...
    <section className="real-mosque-cta" aria-labelledby="mosque-cta-title"><div><p className="real-eyebrow">Services pour les mosquées</p><h2 id="mosque-cta-title">Un espace simple pour accompagner les familles de votre communauté.</h2><p>Centralisez les classes, les tuteurs, le suivi des élèves et les activités extrascolaires avec une supervision humaine claire.</p></div><Link href="/services/mosquees" className="real-button real-button-dark real-button-large">Découvrir les services →</Link></section>
